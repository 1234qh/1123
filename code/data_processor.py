...
class DataProcessor(object):
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        ...
        # Strict temporal comparisons need frame identity to survive voxel
        # generation.  Voxelizing all sweeps together makes current/history
        # points compete for the same MAX_POINTS_PER_VOXEL slots, so the
        # later temporal block cannot recover the discarded frame.  Generate
        # one independent voxel set per sweep instead; Dataset.collate_batch
        # already supports list-valued voxel arrays and packs them as B*S
        # sparse samples.
        if config.get('SEPARATE_SWEEPS', False):
            num_sweeps = int(config.get('NUM_SWEEPS', 1))
            sweep_col = int(config.get(
                'SWEEP_ID_INDEX', max(0, self.num_point_features - 2)
            ))
            if points.ndim != 2 or points.shape[1] <= sweep_col:
                raise RuntimeError(
                    f'SEPARATE_SWEEPS requires sweep id column {sweep_col}, '
                    f'got points shape {points.shape}'
                )
            sweep_ids = np.rint(points[:, sweep_col]).astype(np.int64)
            voxels_list, voxel_coords_list, voxel_num_points_list = [], [], []
            max_points = int(config.MAX_POINTS_PER_VOXEL)
            for sweep_id in range(num_sweeps):
                sweep_points = points[sweep_ids == sweep_id]
                if sweep_points.shape[0] == 0:
                    voxels = np.zeros(
                        (0, max_points, points.shape[1]), dtype=points.dtype
                    )
                    coordinates = np.zeros((0, 3), dtype=np.int32)
                    num_points = np.zeros((0,), dtype=np.int32)
                else:
                    voxels, coordinates, num_points = (
                        self.voxel_generator.generate(sweep_points)
                    )
            ...
            data_dict['separated_sweeps'] = True
